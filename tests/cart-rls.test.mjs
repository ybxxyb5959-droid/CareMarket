...
test('cart migration and two-user RLS in local PostgreSQL', { skip: !process.env.PGLITE_MODULE }, async (t) => {
  ...
  await t.test('stock 0 / 1 / 3 limits cover RPC and direct writes', async () => {
    await db.exec('reset role; update products set stock=3 where product_id=101; update products set stock=1 where product_id=202;')
    await db.exec(fs.readFileSync(new URL('../supabase/migrations/20260906000400_cart_stock_limit.sql', import.meta.url), 'utf8'))
    await asUser(a)
    await db.query('select public.add_my_cart_item(101, 3, $1)', [a])
    await assert.rejects(rpcAdd(a), /available stock/)
    await assert.rejects(db.query('select public.change_my_cart_quantity(101, 1, $1)', [a]), /available stock/)
    await assert.rejects(db.query('update cart_items set quantity=4 where product_id=101'), /available stock/)
    await assert.rejects(db.query('insert into cart_items(user_id,product_id,quantity) values ($1,202,2)', [a]), /available stock/)
    await rpcAdd(a, 202)
    await assert.rejects(rpcAdd(a, 202), /available stock/)
    await db.exec('reset role; update products set stock=0 where product_id=101;')
    await asUser(b)
    await assert.rejects(rpcAdd(b), /available stock/)
    await assert.rejects(db.query('insert into cart_items(user_id,product_id,quantity) values ($1,101,1)', [b]), /available stock/)
    await asUser(a)
    await db.query('delete from cart_items where product_id=101')
  })

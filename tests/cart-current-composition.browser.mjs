...
try {
  ...
  assert.equal(await page.getByRole('button', { name: '주문하기', exact: true }).isEnabled(), true)
  ...
  await page.getByRole('heading', { name: '주문/결제', exact: true }).waitFor()
  await page.goto(origin + '/orders')
  await page.getByRole('heading', { name: '주문 · 배송 내역', exact: true }).waitFor()
  await page.goto(origin + '/products')
  await page.locator('.card-media').first().click()
  await page.getByRole('button', { name: '장바구니 담기', exact: true }).waitFor()
  ...
  console.log(JSON.stringify({stage:'production',drawer:true,cart:true,quantityRefresh:true,emptyCart:true,guest:true,checkout:true,orders:true,productDetail:true,allergyPriority:true,goalIgnored:true,analysisCalls,pageErrors}))

...
function AdminInquiriesContent() {
  ...
  return <>
    <div className="wrap page admin-partnerships-page admin-inquiries-page">
      ...
      {loading ? <div className="empty" role="status"><p>문의 목록을 불러오는 중입니다.</p></div> : error ? <div className="empty" role="alert"><h3>문의 목록을 불러오지 못했습니다.</h3><button className="btn btn-primary btn-sm" onClick={() => void load()}>다시 시도</button></div> : visibleInquiries.length === 0 ? <div className="empty"><h3>{inquiries.length === 0 ? '접수된 1:1 문의가 없습니다.' : '현재 조건에 해당하는 문의가 없습니다.'}</h3></div> : <div className="table-wrap admin-inquiry-table-wrap"><table className="admin-inquiry-table"><thead><tr><th>문의 유형</th><th>제목</th><th>고객 이메일</th><th>주문번호</th><th>접수일</th><th>상태</th><th>관리</th></tr></thead><tbody>{visibleInquiries.map((inquiry) => <tr key={inquiry.id}><td><span className="td-cat">{inquiry.category}</span></td><td><div className="td-name">{inquiry.title}</div></td><td>{inquiry.contact_email}</td><td className="td-mono">{inquiry.orders?.toss_order_id || inquiry.order_id || '—'}</td><td className="admin-date">{formatDate(inquiry.created_at)}</td><td><StatusBadge status={inquiry.status} /></td><td><button className="admin-product-action edit" onClick={() => setSelectedId(inquiry.id)}>보기</button></td></tr>)}</tbody></table></div>}
    </div>

...
export function cartQuickSummary(insight) {
  ...
  if (insight?.basis?.composition_only && insight.composition?.attention) {
    checks.push({ id: 'composition', name: '상품 구성', reason: insight.composition.attention, allergy: false })
  }
  ...
  return { summary: insight?.shortSummary || summary, metrics: selectedMetrics, goodPoint: insight?.goodPoints?.[0] || null,
    ...
    basis: insight?.basis?.composition_only ? '현재 장바구니 구성' : insight?.basis?.primary_goal || '일반 구성', itemCount: products.length }
}

...
|---|---:|---:|---|
| 저당 | 당류 <= 5g | 기존 목록 필터와 동일 | 공통 기준 사용 |
| 고단백 | 단백질 >= 15g | 27 | 간식 4/10, 간편식 7/10 |
| 저염 | 나트륨 <= 250mg | 89 | 간편식 4/10 |

숫자 없는 저당은 기존 일반 필터와 동일한 5g입니다. 저당 5g, 고단백 15g, 저염 250mg은 `_shared/nutrition-policy.js`를 일반 필터·상품 태그·AI 검색·장바구니 분석에서 공유합니다. 사용자가 명시한 영양 수치는 기본 기준보다 우선하며 AI chip에는 실제 적용 숫자를 표시합니다. 저염만으로는 많은 건강식품이 해당하므로 상품군이 함께 들어오면 그 교집합으로 좁힙니다.
